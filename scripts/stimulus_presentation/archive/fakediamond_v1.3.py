...
window = visual.Window(size=(1920,1080), units='pix', color='black', fullscr=True)
word = visual.TextStim(window, text='', font=font, color='lightgrey', anchorHoriz=anchorHoriz, height=30)
probe = visual.TextStim(window, text='', font=font, color='cyan', anchorHoriz=anchorHoriz, wrapWidth=500, height=30)
word_instruction = visual.TextStim(window, text='', font=font, color='lightgrey', anchorHoriz=anchorHoriz, wrapWidth=600, height=25)
...
def present_trial(trial, run='', logfile=None):
    ...
    if run == 'prac':
        ...
        if trial['probe'] != '':
            response, RT = present_probe(trial['probe'])[0]
            # response = present_probe(trial['probe'])
            answer = ''
            if trial['answer'] == 'yes':
                answer = 'Rb'
...
if expInfo['Run'] == 'prac':
    ...
    instructions = ['In this study, you will read phrases word-by-word at the screen centre.',
                    'Please fixate on the cross and read the phrases carefully.',
                    'You will be presented with some yes/no questions. Try and answer as quickly and accurately as you can.',
