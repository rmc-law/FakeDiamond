...
def present_fix():
    for frame in range (12): # refresh rate 60Hz; on 300ms
        if frame <= 12:
            fixation.draw()
        window.flip()
...
def present_word(trigger=None, photoDiode=True):
    ...
    for frame in range(60): # presents each word: on 500ms, off 500ms
        if frame < 30:
            word.draw()
            if photoDiode:
                photodiode.draw()
...
def present_probe(text=''):
    ...
    event.waitKeys(keyList=['1','2'], timeStamped=RT_clock)
    window.flip()
    # return event.waitKeys(keyList=['1','2'], timeStamped=RT_clock)
    # return button_pressed, t

def present_feedback(text=''):
    for frame in range (36): # presents feedback: on 300ms, off 300ms
        if frame <= 18:
            word.draw()
        window.flip()
...
if expInfo['Run'] == 'prac':
    ...
    instructions = ['In this study, you will read phrases word-by-word at the centre of the screen.',
                    ...
                    'You may be presented with a yes/no question, which you answer as quickly and accurately as you can.',
                    'Respond with your INDEX finger for YES,\n and MIDDLE finger for NO.',
                    'We will start with a practice session, and you will be given feedback.']
