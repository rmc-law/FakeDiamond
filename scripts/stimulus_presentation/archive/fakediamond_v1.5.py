...
word_instruction = visual.TextStim(window, text='', font=font, color='lightgrey', anchorHoriz=anchorHoriz, wrapWidth=600, height=20)
fixation = visual.TextStim(window, text='+', font=font, color='lightgrey', height=50)
photodiode = visual.Rect(window, units='height', width=0.08, height=0.05, pos=(-0.85,-0.45), fillColor='grey')
...
def present_fix():
    for frame in range (12): # refresh rate 60Hz; on 200ms
        if frame <= 12:
            fixation.draw()
        window.flip()
    # fixation.draw()
...
def present_word(trigger=None, photoDiode=False):
    # if trigger is not None:
    #     window.callOnFlip(MEG.set_trigger_state, value=trigger, return_to_zero_ms=20)
    for frame in range(30): # presents each word: on 200ms, off 300ms
        if frame < 12:
            word.draw()
            if photoDiode:
                photodiode.draw()
...
def present_probe(text=''):
    ...
    probe.draw()
    window.flip()
    # button_pressed, t = MEG.wait_for_button_press(allowed=['Rb','Ry'], timeout=None) # Ry = right yellow
    return event.waitKeys(keyList=['1','2'], timeStamped=RT_clock, maxWait=3.)
    # return button_pressed, t

def present_feedback(text=''):
    for frame in range (30): # presents feedback: on 300ms, off 300ms
        if frame <= 12:
            word.draw()
        window.flip()
...
if expInfo['Run'] == 'prac':
    ...
    instructions = 'In this study, you will read phrases word-by-word at the screen centre. Please fixate on the cross and read the phrases carefully. \n\nYou will be presented with some yes/no questions. Try and answer as quickly and accurately as you can. Press the BLUE button for YES, and YELLOW for NO. \n\nWe will start with a practice session. You will be given feedback.'
    present_instruction(instructions)

    present_instruction(f'This will be a practice block.\n\n<Click to begin>')
...
# experimental session
    ...
    blocks = np.array_split(stimuli, 5) 
