...
photodiode = visual.Rect(window, units='height', width=0.15, height=0.05, pos=(-0.85,-0.45), fillColor='white')
...
def present_trial(trial, run='', logfile=None):
    ...
    # present probe if the trial has one
    if trial['probe'] != '':
        ...
        if run == 'prac':
            ...
            if response == answer: 
                ...
                core.quit()
        clear_screen()
        core.wait(random.uniform(1.3,1.7)) # intertrial interval: 1.5s with probe
    else: 
        clear_screen()
        core.wait(random.uniform(0.8,1.2)) # intertrial interval: 1s without probe
