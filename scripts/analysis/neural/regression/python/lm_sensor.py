...
for subject_id in subject_ids:
    ...
    subject_output_dir = op.join(output_dir, analysis, 'sensor', subject)
    if op.exists(subject_output_dir):
        ...
        if experiment == 'compose':
            ...
            covariates_vars = ['zipf_noun']#,'zipf_adj']#,'length_adj','length_noun']#,'freq_seq'] 
            # predictor_zipf_adj = [0. if np.isnan(z) else z for z in zscore(epochs.metadata[['zipf_adj']].values.ravel()).tolist()]
            ...
            # predictor_freq_seq = zscore(epochs.metadata[['freq_seq']].values.ravel()).tolist()
            predictors_covariates = [predictor_zipf_noun]#, predictor_zipf_adj]#, predictor_length_adj, predictor_length_noun]#, predictor_freq_seq]
        predictor_intercept = [1] * len(epochs)

        if analysis == 'compose':
            predictor_names = covariates_vars + ['intercept','composition','concreteness','denotation',
                                                 'composition:concreteness']#,'composition:denotation']#,'denotation:concreteness']#'composition:concreteness:denotation']
            ...
            predictors_all = [*predictors_covariates, predictor_intercept, predictor_composition, predictor_concreteness, predictor_denotation,
                              predictor_compositionXconcreteness]#, predictor_compositionXdenotation]#, predictor_denotationXconcreteness]#, predictor_interaction_full]
        ...
        reg = linear_regression(epochs, design_matrix=design, names=predictor_names)
        for predictor in predictor_names: 
            if predictor != 'intercept':
                reg[predictor].beta.save(op.join(subject_output_dir, f'betas_sensor_{predictor}-ave.fif'), overwrite=True, verbose=False)
