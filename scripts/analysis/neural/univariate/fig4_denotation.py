...
FIG_WIDTH = 5.
FIG_FONT_RATIO = 7.5 / 6 # just our defaults
TINY_SIZE = 6 / FIG_FONT_RATIO
SMALL_SIZE = 8 / FIG_FONT_RATIO
MEDIUM_SIZE = 10 / FIG_FONT_RATIO
BIGGER_SIZE = 12 / FIG_FONT_RATIO
...
color_scheme = {
    'concrete-subsective': plt.cm.Reds(0.45),
    'concrete-privative': plt.cm.Reds(0.6),
    'abstract-subsective': plt.cm.Blues(0.45),
    'abstract-privative': plt.cm.Blues(0.6)
}
...
fig, ax_dict = plt.subplot_mosaic(
    ...
    figsize=(FIG_WIDTH, 1.625),  # Set the size of the figure in inches
    ...
    gridspec_kw={
        ...
        'width_ratios': [2,0.75], # Set the relative widths of the columns
        'wspace': 0.001,
        'hspace': 0.005}
)
...
for relevant_condition, color in zip(relevant_conditions,colors_tc):
    ...
    axis.plot(times, data_group_avg.x, color=color, lw=2.5, label=relevant_condition)
...
for i, axis in zip(range(sign_clusters.n_cases), [ax_dict['A']]):
...
# PANEL B: Bar plots for left ATL cluster
axis = ax_dict['B']
...
# 9. FINALIZE & SAVE
out_fname = op.join(figures_dir, 'fig4_denotation.png')
