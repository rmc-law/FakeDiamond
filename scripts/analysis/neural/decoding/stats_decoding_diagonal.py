...
layout_config = {
    ...
    'specificity_word': (1, 1, (6, 2.5), False, False),
    # fallback/default
...
classifier = 'logistic'
window = input('single or sliding: ')
micro_ave = True
if micro_ave:
...
sfreq = int(scores_group[0].shape[1] / 1.6) if to_plot in ['denotation+concreteness'] else int(scores_group[0].shape[1] / 0.8)
...
for i, analysis in enumerate(analyses):
    ...
    if cluster_pvals: # Check if the list of p-values is not empty
        ...
        for cluster, p_val in zip(good_clusters, cluster_pvals):
            if analysis.split('_')[-1] in ['subsective','privative','testSub','testPri','word']:
            ...
            cluster_stats.append([p_val, cluster_start_time, cluster_stop_time])
            print(f"  Found significant cluster for '{analysis}': p={p_val:.4f}, extent={cluster_start_time:.3f}s - {cluster_stop_time:.3f}s")
