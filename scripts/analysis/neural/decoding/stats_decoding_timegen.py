...
parser.add_argument('--to_plot', type=str, required=True, help='Analyses to plot (denotation+concretenss, concretess_xcond)')
parser.add_argument('--data_type', type=str, required=True, help='Sensor or source data (MEEG, ROI)')
...
sfreq = int(scores_group[0].shape[1] / 1.6) if to_plot in ['denotation+concreteness','specificity_word'] else int(scores_group[0].shape[1] / 0.8)
print('sfreq:', sfreq)
...
for i, analysis in enumerate(analyses):
    ...
    # for j in range(scores_group[i].shape[0]):
    #     print(np.mean(scores_group[i][j]))
    #     good_clusters_j, cluster_pv_j, _ = permutation_test_single_subj(scores_group[i][j], t_threshold=None, p_val_cluster_alpha=0.05)
    #     if cluster_pv:
    #         cluster_stats = []
    #         for i, (cluster, pval) in enumerate(zip(good_clusters_j, cluster_pv_j)):
    #             if pval < 0.05:
    #                 # Off-diagonality calculation
    #                 T = scores_group[i][j]  # assumes square TGM
    #                 mask = cluster.reshape(T, T)  # Boolean matrix [T, T]
    #                 train_inds, test_inds = np.where(mask)
    #                 off_diag_dist = np.abs(train_inds - test_inds)
    #                 mean_offset = np.mean(off_diag_dist)
    #                 max_offset = np.max(off_diag_dist)

    #                 print(f"\nQuantifying how far from diagonal the cluster extends:")
    #                 print(f"  Cluster {i + 1}: p={pval:.4f}, mean offset={mean_offset:.2f} ticks ({mean_offset / sfreq:.3f} s), "
    #                     f"max offset={max_offset} ticks ({max_offset / sfreq:.3f} s)\n")

    #                 # Save stats: cluster index, p-value, mean offset (s), max offset (s)
    #                 cluster_stats.append([i + 1, pval, mean_offset / sfreq, max_offset / sfreq])

    #     # Save to file
    #     stats_filename = op.join(figures_dir, f'stats_cluster_pvals_{analysis}_{roi}_sub{j+1}')
    #     if micro_ave:
    #         stats_filename += '_micro-ave.txt'
    #     else:
    #         stats_filename += '.txt'

    #     header = (f'Cluster-level stats for {analysis} in {roi}\n'
    #             f'Columns: cluster_index, p_value, mean_offset_from_diagonal_sec, max_offset_from_diagonal_sec')
    #     np.savetxt(stats_filename, cluster_stats, fmt=['%d', '%.6f', '%.4f', '%.4f'], header=header)
    #     print(f"Saved cluster stats (with off-diagonality) to {stats_filename}.")
    ...
    if cluster_pv:
        ...
        for i, (cluster, pval) in enumerate(zip(good_clusters, cluster_pv)):
            ...
            # Off-diagonality calculation
            mask = cluster  # Boolean matrix [T, T]
            train_inds, test_inds = np.where(mask)
            off_diag_dist = np.abs(train_inds - test_inds)
            mean_offset = np.mean(off_diag_dist)
            max_offset = np.max(off_diag_dist)
