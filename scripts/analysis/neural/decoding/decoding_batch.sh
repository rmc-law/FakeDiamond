...
for subject in "${subjects[@]}"; do
    ...
    subject_analysis_output_dir="/imaging/hauk/rl05/fake_diamond/scripts/analysis/neural/decoding/output/${analysis}/sub-${subject}"
    
    if [ ! -e "$subject_analysis_output_dir" ]; then
        echo "Output of $analysis decoding does not exist for sub-$subject. Decoding."
        sbatch --export=subject="$subject",analysis="$analysis",classifier="$classifier" decoding_job.sh
    else
